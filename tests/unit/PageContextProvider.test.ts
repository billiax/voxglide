...
describe('PageContextProvider', () => {
  ...
  describe('fields without id or name', () => {
    it('generates fallback ID for input elements without id or name', async () => {
      document.body.innerHTML = `
      ...
      // Standalone inputs now get fallback IDs using type:placeholder
      const formSection = result.content.split('Form Fields')[1]?.split('\n') || [];
      const anonymousLine = formSection.find((line: string) => line.includes('text:anonymous'));
      expect(anonymousLine).toBeDefined();
    });

    it('uses type:unnamed for inputs without id, name, or placeholder', async () => {
      document.body.innerHTML = `
        <input type="email" />
      `;
      provider = new PageContextProvider(true);
      ...
      expect(result.content).toContain('email:unnamed');
    });
  });
});
