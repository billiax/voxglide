...
export class InteractiveElementScanner {
  ...
  private cursorPointerCount = 0;
  private lastScanDuration = 0;
  private indexMap = new Map<number, HTMLElement>();
  private nextIndex = 1;

  constructor(maxElements = 100, excludeSelectors: string[] = []) {
  ...
  scan(): InteractiveElement[] {
    ...
    let visibleCount = 0;
    const excludeSelector = this.excludeSelectors.length > 0 ? this.excludeSelectors.join(', ') : null;
    ...
    for (const el of rawElements) {
      ...
      // Skip elements inside SDK shadow DOM or that ARE the SDK host
      if (this.isSDKElement(htmlEl)) continue;

      // Skip excluded selectors
      if (excludeSelector && htmlEl.closest(excludeSelector)) continue;
    ...
    this.selectorCount = visibleCount;
    ...
    const cursorPointerAdded = this.scanCursorPointerElements(selectorMatched, results);
    visibleCount += cursorPointerAdded;
    this.cursorPointerCount = cursorPointerAdded;
    ...
    this.lastScanDuration = performance.now() - scanStart;
   ...
   */
  getScanMetadata(): import('../types').ScanMetadata {
    return {
      scanDuration: this.lastScanDuration,
      elementCount: this.includedCount,
      truncated: this.wasTruncated,
      totalFound: this.totalFound,
      phaseCounts: { selector: this.selectorCount, cursorPointer: this.cursorPointerCount },
    };
  }

  /**
   * Check if an element belongs to the SDK (light DOM or Shadow DOM).
   */
  private isSDKElement(el: HTMLElement): boolean {
    if (el.closest('[data-voice-sdk]')) return true;
    const root = el.getRootNode();
    if (root instanceof ShadowRoot && (root.host as HTMLElement)?.closest('[data-voice-sdk]')) return true;
    return false;
  }
  ...
  private scanCursorPointerElements(
    ...
    for (const node of allElements) {
      ...
      // Skip SDK elements
      if (this.isSDKElement(el)) continue;
  ...
  private generateDescription(el: HTMLElement): string {
    ...
    // Icon-only button detection — buttons with only SVG/img children and no text
    if (tag === 'button' || role === 'button' || role === 'menuitem') {
      const children = Array.from(el.children);
      const isIconOnly = children.length > 0 && children.every(c => {
        const ct = c.tagName.toLowerCase();
        return ct === 'svg' || ct === 'img' || ct === 'path' || ct === 'i'
          || (ct === 'span' && !c.textContent?.trim());
      });
      if (isIconOnly) {
        const section = el.closest('section, article, details, fieldset, [class*="card"], [class*="panel"]');
        const heading = section?.querySelector('h1,h2,h3,h4,h5,h6');
        if (heading?.textContent?.trim()) {
          return this.truncate(`[icon button] in: ${heading.textContent.trim()}`, 60);
        }
        const prev = el.previousElementSibling;
        if (prev?.textContent?.trim()) {
          return this.truncate(`[icon button] near: ${prev.textContent.trim()}`, 60);
        }
        return '[icon button]';
      }
    }
