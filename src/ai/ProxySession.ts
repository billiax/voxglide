...
export class ProxySession {
  ...
  private handleMessage(msg: any, onReady?: (value: void) => void): void {
    switch (msg.type) {
      ...
      case 'error':
        // Server-sent errors are LLM/processing failures — the WebSocket is still connected.
        // Show the error in transcript so user sees it, but don't call onError (which sets
        // connection state to ERROR and can break mic/input). The user can simply try again.
        this.callbacks.onTranscript(`Error: ${msg.message || 'Server error'}`, 'ai', true);
        break;
    }
  }
  ...
  private notifySpeechState(): void {
    const actuallyListening = this.speechCapture?.isActuallyListening() ?? false;
    const running = this.speechCapture?.isRunning() ?? false;
    const paused = this.speechCapture?.isPaused() ?? false;
    // Report actuallyListening (real browser state) as 'active' to the UI,
    // and 'running' (user intent) separately so toggle() can distinguish
    // "speech failed, offer retry" from "user stopped, offer start".
    this.callbacks.onSpeechStateChange?.(actuallyListening, paused, running);
  }
